...
export default function OrderTracking({
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      const mapped: GridOrder[] = Array.isArray(raw)
        ? raw.map((o, idx) => ({
          id: o.id,
          level: idx + 1,
      ...
      // Don't show error toast for empty/missing orders — just show empty state
      console.warn('[OrderTracking] Failed to load orders:', e);
      setOrders([]);
    } finally {
