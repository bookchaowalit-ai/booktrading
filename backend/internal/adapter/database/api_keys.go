...
// using HKDF-SHA256 with a fixed salt for proper key derivation.
// Panics if the env var is not set or too short.
func encryptionKey() []byte {
	raw := os.Getenv("ENCRYPTION_KEY")
	if raw == "" {
		panic("ENCRYPTION_KEY environment variable is not set. Set a 32+ character random string before starting the application.")
	}
	if len(raw) < 32 {
		panic("ENCRYPTION_KEY must be at least 32 characters long for security.")
	}

	// Use HKDF-SHA256 with a fixed salt for key derivation
	salt := []byte("trading-bot-api-key-encryption")
	key := make([]byte, 32)
	h := hkdf.New(sha256.New, []byte(raw), salt, nil)
