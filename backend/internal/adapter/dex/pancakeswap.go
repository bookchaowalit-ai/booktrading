...
func (p *PancakeSwapProvider) CheckAllowance(ctx context.Context, tokenAddress, owner, spender string) (*big.Int, error) {
...
func (p *PancakeSwapProvider) ApproveToken(ctx context.Context, params *ApproveParams) (*SwapResult, error) {
	...
	logger.Info("PancakeSwap approve", "token", params.TokenAddress, "spender", params.Spender, "gas", gasEst)
...
func (p *PancakeSwapProvider) GetLiquidityPools(ctx context.Context, userAddress string) ([]LiquidityPosition, error) {
	...
	for i := uint64(0); i < count; i++ {
		...
		resData, _ := reserveABI.Pack("getReserves")
		...
		reserveABI.UnpackIntoInterface(&reserves, "getReserves", resResult)
